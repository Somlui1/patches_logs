...
$results = foreach ($row in $rawObjects) {
    ...
    foreach ($col in $headerMap.GetEnumerator()) {
        ...
        if ($origName -eq 'Date' -and $value -is [string]) {
            # Try to parse; if successful use formatted date, otherwise use original
            try {
                $dt = [datetime]::ParseExact($value, "yyyy-MM-dd HH:mm:ss", [System.Globalization.CultureInfo]::InvariantCulture)
                $value = $dt
                # Add formatted date to key parts
                $null = $keyParts.Add($dt.ToString("yyyyMMddHHmmss"))
            } catch {
